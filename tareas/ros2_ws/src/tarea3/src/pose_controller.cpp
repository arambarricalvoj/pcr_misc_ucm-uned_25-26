...
#include "tarea3/service_utils.hpp"
#include "tarea3/pose_utils.hpp"
...
: Node("pose_controller")
{
    ...
    pose_sub_ = create_subscription<geometry_msgs::msg::Pose>(
        ...
        std::bind(&PoseController::robotPoseCallback, this, std::placeholders::_1));

    // Publicador
    cmd_pub_ = create_publisher<geometry_msgs::msg::Twist>("/master_khepera_iv/cmd_vel", 10);

    // Cliente del servicio
    client_ = create_client<std_srvs::srv::Trigger>("/master_khepera_iv/get_target_pose");

    // Action server
    action_server_ = rclcpp_action::create_server<ExecuteTrajectory>(
        this,
        "/master_khepera_iv/execute_trajectory",
        std::bind(&PoseController::handleGoal, this, std::placeholders::_1, std::placeholders::_2),
        std::bind(&PoseController::handleCancel, this, std::placeholders::_1),
        std::bind(&PoseController::handleAccepted, this, std::placeholders::_1)
    );

    // Timer del control
    timer_ = create_wall_timer(
        50ms,
        std::bind(&PoseController::controlLoop, this));
}
...
rclcpp_action::GoalResponse PoseController::handleGoal(
...
rclcpp_action::CancelResponse PoseController::handleCancel(
...
void PoseController::handleAccepted(
    const std::shared_ptr<GoalHandleExecuteTrajectory> goal_handle)
{
    ...
    if (target_is_empty)
    {
        ...
        requestTargetPositionAsync(
            client_,
            std::bind(&PoseController::onTargetReceived, this, std::placeholders::_1),
            get_logger()
        );
    }
    else
...
void PoseController::onTargetReceived(
...
void PoseController::robotPoseCallback(const geometry_msgs::msg::Pose::SharedPtr msg)
{
    robot_pose_ = *msg;
    pose_received_ = true;
}
...
void PoseController::controlLoop()
{
